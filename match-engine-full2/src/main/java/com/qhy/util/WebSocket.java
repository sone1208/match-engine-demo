...
import com.qhy.util.ServerEncoder;
...
public class WebSocket {
    ...
    /**
     * 当某用户委托订单状态变化时，将新的列表广播给为对应用户的session连接
    ...
    public void sendNewTakerOrders(List<Order> takerOrders, Integer userId) throws IOException {
        ...
        if (sessionIds != null) {
            for (String sessionId : sessionIds) {
                log.info("发送消息：" + sessionId + "----" + "takerOrders");
                Session session = clientMap.get(sessionId);
                session.getAsyncRemote().sendObject(new Results("takerOrders", takerOrders));
            }
        }
    }
